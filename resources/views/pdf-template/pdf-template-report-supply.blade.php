...
<div style="font-family: sans-serif; overflow: auto;">
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 25px; font-size: 12px;">
        <tr>
            <th style="border-bottom: 1px solid #eee; padding: 10px; text-align: left; color: #666; width: 25%;">Total Supplies</th>
        ...
        <tr>
            <td style="padding: 15px 10px;">
                <div class="text-blue" style="font-size: 16px; font-weight: bold;">{{ $totalSupply }}</div>
    ...
    @if($mostBorrowed && $mostBorrowed->borrow_count > 0)
        <div style="border: 1px solid #eee; margin-bottom: 25px; font-size: 12px; border-radius: 4px;">
            ...
            <div style="padding: 12px; color: #444; line-height: 1.4;">
                ...
                with a total of <strong class="text-green">{{ $mostBorrowed->borrow_count }}</strong> reservation requests.
            </div>
        </div>
    @endif
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 25px;">
        <thead>
            <tr style="background-color: #fcfcfc;">
                <th style="border-bottom: 1px solid #eee; padding: 12px 11px; text-align: left; color: #555;">Supply Name</th>
        ...
        <tbody>
            @foreach($supplies as $supply)
                <tr>
                    <td style="border-bottom: 1px solid #f5f5f5; padding: 10px; font-weight: bold;">{{ $supply->item_name }}</td>
                    <td style="border-bottom: 1px solid #f5f5f5; padding: 10px; color: #666;">{{ $supply->location }}</td>
                    <td style="border-bottom: 1px solid #f5f5f5; padding: 10px; text-align: center; font-weight: bold;">{{ $supply->quantity }}</td>
                    <td class="text-green" style="border-bottom: 1px solid #f5f5f5; padding: 10px; text-align: center; font-weight: bold;">{{ $supply->available }}</td>
                    <td class="text-yellow" style="border-bottom: 1px solid #f5f5f5; padding: 10px; text-align: center; font-weight: bold;">{{ $supply->reserved }}</td>
                    <td class="text-red" style="border-bottom: 1px solid #f5f5f5; padding: 10px; text-align: center; font-weight: bold;">{{ $supply->unavailable_qty }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
                ...
                @foreach($outOfStock as $supply)
                    <tr style="border-bottom: 1px solid #f5f5f5;">
                        <td style="padding: 10px;">
                            ...
                            <strong style="vertical-align: middle; font-size: 12px;">{{ $supply->item_name }}</strong>
                        </td>
                        <td style="padding: 10px; text-align: center; font-weight: bold;" class="text-red">0 / {{ $supply->quantity }}</td>
                        <td style="padding: 10px; text-align: center; font-weight: bold;" class="text-red">0%</td>
                        <td style="padding: 10px; color: #666;">{{ $supply->location }}</td>
                    </tr>
                @endforeach
                ...
                @foreach($criticalStock as $supply)
                    <tr style="border-bottom: 1px solid #f5f5f5;">
                        <td style="padding: 10px;">
                            ...
                            <strong style="vertical-align: middle;">{{ $supply->item_name }}</strong>
                        </td>
                        <td style="padding: 10px; text-align: center; font-weight: bold;">{{ $supply->available }} / {{ $supply->quantity }}</td>
                        <td style="padding: 10px; text-align: center; font-weight: bold;" class="text-orange">{{ round($supply->availability_percentage, 0) }}%</td>
                        <td style="padding: 10px; color: #666;">{{ $supply->location }}</td>
                    </tr>
                @endforeach
                ...
                @foreach($lowStock as $supply)
                    <tr style="border-bottom: 1px solid #f5f5f5;">
                        <td style="padding: 10px;">
                            ...
                            <strong style="vertical-align: middle; font-size: 11px;">{{ $supply->item_name }}</strong>
                        </td>
                        <td style="padding: 10px; text-align: center; font-weight: bold;">{{ $supply->available }} / {{ $supply->quantity }}</td>
                        <td style="padding: 10px; text-align: center; font-weight: bold;" class="text-yellow">{{ round($supply->availability_percentage, 0) }}%</td>
                        <td style="padding: 10px; color: #666;">{{ $supply->location }}</td>
                    </tr>
                @endforeach
            </tbody>
